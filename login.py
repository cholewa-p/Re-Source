...
def login():
    ...
    cur = conn.cursor()
    cur.execute("SELECT account_id, username, password_hash, role FROM user_accounts WHERE username = %s", (username,))
    user = cur.fetchone()
    conn.close()
    ...
    session['role'] = user.get('role', 'user') if isinstance(user, dict) else user['role'] if 'role' in user else 'user'
...
def me():
    ...
    return jsonify({"user_id": session['user_id'], "username": session['username'], "role": session.get('role','user')}), 200