...
@db_bp.route('/update_ts_data', methods=['POST'])
def update_ts_data():
    """Ingest time series points posted as JSON array.
    ...
    try:
        ...
        if role == 'admin':
            ...
            if source_ids:
                ...
                cur.execute("SELECT source_id FROM energy_sources WHERE source_id = ANY(%s)", (source_ids,))
                ...
                model, fc = train_aggregated_model(db_cfg, source_ids, start=start_dt, end=end_dt, horizon_hours=horizon_int)
                scope = f'sources_{"_".join(map(str, source_ids))}'
            elif source_id is not None:
                model, fc = train_electricity_model(db_cfg, source_id, start=start_dt, end=end_dt, horizon_hours=horizon_int)
                scope = f'source_{source_id}'
            else:
                # all sources
                cur.execute("SELECT source_id FROM energy_sources")
                sid_list = [r['source_id'] for r in cur.fetchall()]
                model, fc = train_aggregated_model(db_cfg, sid_list, start=start_dt, end=end_dt, horizon_hours=horizon_int)
                scope = 'all_sources'
        else:
            # Normal user path
            ...
            if source_ids:
                ...
                missing = set(source_ids) - found
                if missing:
                    return jsonify({"error": f"Sources not found or not owned: {sorted(missing)}"}), 404
                model, fc = train_aggregated_model(db_cfg, source_ids, start=start_dt, end=end_dt, horizon_hours=horizon_int)
...
def save_model():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"message": "Model saved to disk, but DB record failed", "path": rel_file}), 200

        return jsonify({"message": "Model saved", "path": rel_file, "model_id": saved_model_id}), 200
    except Exception:
